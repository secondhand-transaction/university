     ...
     <ul id="systemSetting"
				...
				<div>
					...
					<ul class="nav nav-pills nav-stacked collapse in " id="sellSetting">						
						<li class="active"><a href="GoToMessagelist.doHe" class="ex0 active"> <i
				...
				<table class="mt">
					<tbody>
						<tr>
							...
							<tr>
								<td>${messageList.getSeller_name()}</td>								
								<td>${messageList.getDate()}</td>
								<td><c:if test="${messageList.getStatus() eq '1'}">已读</c:if>
									<c:if test="${messageList.getStatus() eq '0'}">未读</c:if>
								</td>
								<td><a href="GoToMessage.doHe?id=${messageList.getSeller_id()}">回复</a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
